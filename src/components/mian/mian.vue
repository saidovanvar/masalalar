<template>
  <main class="bg-[#373737] text-white p-6 mt-[555px] min-h-screen">
    <!-- Navigation -->
    <nav class="flex flex-wrap justify-center md:justify-start gap-6 text-sm uppercase text-gray-400 mb-6">
      <span class="hover:text-red-500 text-white cursor-pointer">Кухонные ножи</span>
      <span class="hover:text-red-500 text-white cursor-pointer">Складные ножи</span>
      <span class="hover:text-red-500 text-white cursor-pointer">Точилки для ножей</span>
      <span class="hover:text-red-500 text-white cursor-pointer">Аксессуары для кухни</span>
    </nav>

    <!-- Grid -->
    <div class="grid grid-cols-1 sm:grid-cols-4 gap-6">
      <!-- Banner -->
      <div class="relative col-span-1 sm:col-span-2 rounded overflow-hidden">
        <img
            src="../../assets/Rectangle%2033.png"
            alt="Banner"
            class="w-[590px] h-[481px] opacity-80"
        />
        <div class="absolute bottom-4 left-4 text-lg font-medium max-w-xs sm:max-w-none">
          Познакомьтесь с коллекцией ножей <span class="text-red-500">TUOTOWN</span>
        </div>
      </div>
      ...
      >
        <img
            ...
            class="bg-[#2A2A2A] w-[286px] rounded-[14px] h-[412px]  "
        />
        <div class="text-sm font-light pt-[12px]">{{ product.name }}</div>
        <div class="text-xs text-gray-400 mt-1 flex gap-[28px]">
          <span v-if="product.price">{{ formatPrice(product.price) }}</span>
          <span v-if="product.isNew" class="text-[#FF3232] ml-2 font-medium">Новинка</span>
        </div>

        <!-- Button wrapper -->
        <div class="absolute top-[24px] right-[27px]" v-if="product.name">
          <button
              :class="[
              'flex z-10 items-center gap-2 px-3 py-2 rounded-full transition-all duration-300',
              product.showCartIcon ? 'bg-red-600' : 'hover:bg-red-600 w-10 h-10 justify-center'
            ]"
          ...
          <img class="z-20 w-[286px] h-[412px] " v-if="product.isNew" src="../../assets/Insta%20cart.svg" alt="New Product" />
        </div>
      </div>
    </div>
  </main>
  <section class="bg-[#373737] text-white p-6 min-h-screen ">
   <div>
     <h2 class="font-medium text-[27px] text-center	pb-[125px]">
...
>
  ...
  >
    <img
        ...
        class="bg-[#2A2A2A] w-[286px] rounded-[14px] h-[412px]  "
    />
    <div class="text-sm font-light pt-[12px]">{{ pro.name }}</div>
    <div class="text-xs text-gray-400 mt-1 flex gap-[28px]">
      <span v-if="pro.price">{{ formatPrice(pro.price) }}</span>
      <span v-if="pro.isNew" class="text-[#FF3232] ml-2 font-medium">Новинка</span>
    </div>

    <!-- Button wrapper -->
    <div class="absolute top-[24px] right-[27px]" v-if="pro.name">
      <button
          :class="[
              'flex z-10 items-center gap-2 px-3 py-2 rounded-full transition-all duration-300',
              pro.showCartIcon ? 'bg-red-600' : 'hover:bg-red-600 w-10 h-10 justify-center'
            ]"
          @click="addToCard(index)"
          aria-label="Add to cart"
      >
        <svg
            width="24"
...
import knife1 from '../../assets/image 16.svg'
import knife2 from '../../assets/image 17.svg'
import knife3 from '../../assets/image 16.svg'
import knife4 from '../../assets/image 17.svg'
import knife5 from '../../assets/Insta cart.svg'
import Filter from '../mian/filter.vue'
...
function addToCart(index) {
  products.value[index].showCartIcon = true
  setTimeout(() => {
    products.value[index].showCartIcon = false
  }, 145555000)
}
function addToCard(index) {
  pro.value[index].showCartIcon = true
  setTimeout(() => {
    pro.value[index].showCartIcon = false
  }, 145555000)
}
